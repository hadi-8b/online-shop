...
const ProductFilterForm = withFormik<
...
>({
  ...
  handleSubmit: async (values, { setFieldError }) => {
    try {
      const response = await apiClient.post("api/products/filter", values);
      // اگر route شما /api/... است:
      // const response = await apiClient.post("/api/products/filter", values);

      if (response.status) {
        console.log(response.data);
      } else {
        setFieldError("search", response.message || "فیلتر کردن محصولات انجام نشد");
      }
    } catch (error) {
      console.error(error);
      setFieldError("search", "هنگام فیلتر کردن محصولات خطایی روی داد");
    }
  },
